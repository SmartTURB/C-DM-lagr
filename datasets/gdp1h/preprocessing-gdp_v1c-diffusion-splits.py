...
filename_out = 'gdp1h_ve_diffusion_splits.h5'
with h5py.File(filename_out, 'w') as hf:
    hf.create_dataset('min', data=rx0)
    hf.create_dataset('max', data=rx1)
    hf.create_dataset('train', data=x_train[..., :1])
    hf.create_dataset('test',  data=x_test[..., :1])

filename_out = 'gdp1h_vn_diffusion_splits.h5'
with h5py.File(filename_out, 'w') as hf:
    hf.create_dataset('min', data=rx0)
    hf.create_dataset('max', data=rx1)
    hf.create_dataset('train', data=x_train[..., 1:])
